...
;; - `:global' ,but using `keymap-global-set'.
;; - `:option' with a simpler implementation using `setopt'.
;; - Remove `:file-match' in favor of `:mode'.
;; - Improve `:mode' and `:interpreter' to support leaf-style arguments.
...
(setup-define :rebind
  (lambda (key command)
    (if (vectorp key)
      ...
      `(progn
         (dolist (old-key (where-is-internal ',command ,(setup-get 'map)))
           (keymap-set ,(setup-get 'map) old-key nil))
         (keymap-set ,(setup-get 'map) ,key ,command))))
...
(defun setup--process-mode-arg (arg mode)
  "Process ARG for :mode and return cons cells (REGEXP . MODE).
MODE is the default mode to use when not specified in ARG."
  (cond
   ((stringp arg)
    (list (cons arg mode)))
   ...
   (t (error "Invalid :mode argument: %S" arg))))
  ...
  (lambda (arg)
    (macroexp-progn
     (mapcar (lambda (pair)
             ...
             (setup--process-mode-arg arg (setup-get 'mode)))))
  :documentation "Associate the current mode with files matching REGEXP.
Each argument can be:
- A regexp string (uses current mode)
- A cons-cell (REGEXP . MODE) to specify a different mode
- A list of regexps to associate with the current mode
- A cons-cell ((REGEXP1 REGEXP2 ...) . MODE) to associate a list with a mode."
  :debug '(sexp)
  :repeatable t)

(defun setup--process-interpreter-arg (arg mode)
  "Process ARG for :interpreter and return cons cells (INTERPRETER . MODE).
MODE is the default mode to use when not specified in ARG."
  (cond
   ((stringp arg)
    (list (cons arg mode)))
   ...
   ((listp arg)
    (mapcar (lambda (pat) (cons pat mode)) arg))
   (t (error "Invalid :interpreter argument: %S" arg))))

(setup-define :interpreter
  (lambda (arg)
    (macroexp-progn
     (mapcar (lambda (pair)
               `(add-to-list 'interpreter-mode-alist ',pair))
             (setup--process-interpreter-arg arg (setup-get 'mode)))))
  :documentation "Associate the current mode with interpreters matching INTERPRETER.
Each argument can be:
- An interpreter name string (uses current mode)
- A cons-cell (INTERPRETER . MODE) to specify a different mode
- A list of interpreter names to associate with the current mode
- A cons-cell ((INT1 INT2 ...) . MODE) to associate a list with a mode."
  :debug '(sexp)
  :repeatable t)
