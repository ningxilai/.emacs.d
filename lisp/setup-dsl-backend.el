...
(defvar setup-dsl-current-backend :elisp)
...
(defun setup-dsl--setup-body (form)
  "Emit a core term as a form accepted in the uploaded `setup' body." 
  (pcase (car-safe form)
    ...
    (:set (pcase (nth 3 form)
            ...
            (_ `(:option* ,(nth 1 form) ,(nth 2 form)))))
    ...
    (:load-after `(:with-feature ,(nth 1 form) (:when-loaded ,(setup-dsl--setup-body (nth 2 form)))))
    (:defer `(:delay ,(nth 1 form) ,(setup-dsl--setup-body (nth 2 form))))
    (:context `(:with ,@(cdr form)))
    (_ form)))
