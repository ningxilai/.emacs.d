...
(cl-defstruct wm-window buffer (point 0) (start 0) (hscroll 0) dedicated)
...
(defun wm-layout-stacked-columns ()
  (wm-reset-layout)
  (wm-restore-window (cl-first wm-windows))
...
(defun wm-remove-nth (n lst)
  (when lst
    (if (zerop n)
        (rest lst)
      (cons (cl-first lst) (wm-remove-nth (1- n) (rest lst))))))
...
(defun wm-insert-nth (lst n val)
  (if (= n 0)
      (cons val lst)
    (cons (cl-first lst) (wm-insert-nth (rest lst) (1- n) val))))
...
(defun wm-switch-workspace (workspace)
  ...
  (let ((state (cdr (assoc workspace wm-workspaces))))
    ...
    (wm-restore-workspace (cl-first state) (cl-second state) (third state))))
