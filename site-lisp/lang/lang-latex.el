...
(setup LaTeX-mode (:elpaca auctex)
       ...
       ;; font setting
       (:hooks LaTeX-mode-hook (lambda () (progn (buffer-face-set '(:family "JetBrains Mono"))
                                            (buffer-face-mode)
                                            (visual-line-mode)
                                            (auto-fill-mode))))
       ...
       (:with-map latex-mode-map (:bind "M-<down>" 'move-line-region-down
...
(setup (:elpaca preview-auto)
  (:hooks LaTeX-mode-hook preview-auto-setup)
